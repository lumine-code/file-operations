...
class OperationFailure extends Error {
  constructor(message, { effects = [], cleanupPath = null, partial = effects.length > 0 } = {}) {
    super(message);
...
function manifestsEqual(left, right) {
  ...
  for (const [entryPath, expected] of left) {
    ...
    for (const key of ["isDirectory", "isSymbolicLink", "size", "linkTarget", "hash"]) {
...
class Planner {
  ...
  async lookup(filePath, limit = this.actions.length, options = {}) {
    for (let index = limit - 1; index >= 0; index--) {
      ...
      if (pathContains(action.newPath, filePath)) {
        ...
        return this.lookup(translated, index, options);
      }
  ...
  async isCaseOnlyRename(oldPath, newPath, source, target) {
    if (!source || !target || !identitiesEqual(source.runtimeSnapshot, target.runtimeSnapshot)) {
      return false;
    }
...
class FileOperationPlan {
  ...
  async executeNext({ signal } = {}) {
    ...
    try {
      ...
      return {
        ...
        ...(failure.cleanupPath && { cleanupPath: failure.cleanupPath }),
      };
    } finally {
      this.#running = false;
    }
  ...
  async executeStep(step, signal) {
    ...
    try {
      ...
      throw new OperationFailure(failure.message, {
        ...
        cleanupPath: failure.cleanupPath || remainingParents.at(-1)?.path || null,
  ...
  async executeCreate(step, signal) {
    ...
    try {
      ...
      if (!(await removeOwnedPath(stagePath, stageSnapshot))) cleanupPath = stagePath;
      if (backupMoved && !(await removeOwnedPath(backupPath, targetEntry.runtimeSnapshot))) {
      ...
      if (stageSnapshot && !(await removeOwnedPath(stagePath, stageSnapshot))) {
        recoveryPath ||= stagePath;
      }
      entry.runtimeSnapshot = await snapshot(operation.path);
      await this.refreshBindings(step.targetParent);
      throw new OperationFailure(reasonFor(error), {
        effects,
        cleanupPath: recoveryPath,
      });
    }
  }
  ...
  async executeRename(step, signal) {
    ...
    sourceEntry.runtimeSnapshot = await snapshot(operation.newPath);
  ...
  async executeCaseRename(step, signal) {
    ...
    try {
      ...
      await validateSnapshot(temporaryPath, sourceEntry.runtimeSnapshot, { identityOnly: true });
      await validateSnapshot(operation.newPath, null);
      throwIfAborted(signal);
      await fsp.rename(temporaryPath, operation.newPath);
      ...
      if (movedToTemporary) {
        ...
        if (
          ...
          cleanupPath = operation.newPath;
        }
      }
      throw new OperationFailure(reasonFor(error), { effects, cleanupPath });
    }
  }
  ...
  async executeLocalRename(step, signal) {
    ...
    try {
      ...
      throwIfAborted(signal);
      await fsp.rename(operation.oldPath, operation.newPath);
      moved = true;
      await validateSnapshot(operation.newPath, sourceEntry.runtimeSnapshot, {
        identityOnly: true,
      });
      ...
      if (backupMoved && targetEntry) {
        if (!(await restoreMovedPath(backupPath, operation.newPath, targetEntry.runtimeSnapshot))) {
          cleanupPath ||= backupPath;
      ...
      if (error.code === "EXDEV" && effects.length === 0 && !cleanupPath) throw error;
      throw new OperationFailure(reasonFor(error), { effects, cleanupPath });
    }
  }
  ...
  async executeCrossDeviceRename(step, signal) {
    ...
    try {
      ...
      throwIfAborted(signal);
      await fsp.rename(stagingPath, operation.newPath);
      publishedSnapshot = await snapshot(operation.newPath);
      committed = true;
      let cleanupPath = null;
      ...
      if (committed) {
        throw new OperationFailure(reasonFor(error), {
          effects: [renameEffect(operation.oldPath, operation.newPath, sourceEntry.isDirectory)],
          cleanupPath: tombstoneMoved ? tombstonePath : backupMoved ? backupPath : null,
        });
      }
      let cleanupPath = null;
      const effects = [];
