...
async function renameWithoutReplacing(
  ...
  try {
    ...
    if (reservationSnapshot) {
      const state = await pathIdentityState(destinationPath, reservationSnapshot);
      if (state === "owned" && (await removeOwnedPath(destinationPath, reservationSnapshot))) {
...
class FileOperationPlan {
  ...
  async executeCreate(step, signal) {
    ...
    try {
      ...
      if (publishedSnapshot) {
        const state = await pathIdentityState(operation.path, publishedSnapshot);
        if (state === "owned" && !(await removeOwnedPath(operation.path, publishedSnapshot))) {
  ...
  async executeCrossDeviceRename(step, signal) {
    ...
    try {
      ...
      const effects = [...(error.effects || [])];
      if (publishedSnapshot) {
        const state = await pathIdentityState(operation.newPath, publishedSnapshot);
        if (state === "owned" && !(await removeOwnedPath(operation.newPath, publishedSnapshot))) {
      ...
      if (stagingSnapshot) {
        const state = await pathIdentityState(stagingPath, stagingSnapshot);
        if (state === "owned" && !(await removeOwnedPath(stagingPath, stagingSnapshot))) {
