  ...
  async executeNext({ signal } = {}) {
    ...
    try {
      ...
      if (step.skip) {
        ...
        if (step.kind === "rename") {
          documentMove = this.#lifecycle.beginFileMove?.([
            {
              oldPath: step.operation.oldPath,
              newPath: step.operation.newPath,
              isDirectory: step.sourceEntry.isDirectory && !step.sourceEntry.isSymbolicLink,
            },
          ]);
        }
    ...
    try {
      let documentError;
      try {
        await documentMove?.complete(result.effects.filter((effect) => effect.kind === "rename"));
      } catch (error) {
        documentError = error;
      }
...
module.exports = class FileOperationsExecutor {
  constructor({ beginFileMove } = {}) {
    this.lifecycle = new StepLifecycle();
    this.lifecycle.beginFileMove = beginFileMove;
  }
